...
const CommentOther = ({ comment }) => {
    ...
    return (
        <Box bg={bg} className="rounded-2xl pt-1 px-2 w-fit text-left">
            <Box className="flex">
                <CommentAuthor comment={comment}></CommentAuthor>
                <Text className="text-s font-semibold px-3 mt-1"> {comment.text || ''}</Text>
            </Box>
        </Box>
    )
}
...
const CommentSelf = ({ comment }) => {
    ...
    return (
        <Box bg={bg} className="rounded-2xl py-1 px-3 w-fit ml-auto text-right">
            <CommentAuthor comment={comment}></CommentAuthor>
...
const CommentTimestamp = (timestamp) => {
    return <TimeAgo className="text-xs  text-neutral-400" datetime={timestamp.timestamp} />
}
...
const CommentAuthor = ({ comment }) => {
    ...
    const className = useColorModeValue(
        ...
        'text-xs text-neutral-300 font-light'
    )
...
export const Comment = ({ comment }) => {
    ...
    return (
        <Box className="pt-1">
            <CommentField comment={comment}></CommentField>
        </Box>
    )
}
