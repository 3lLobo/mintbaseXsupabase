...
const Post = ({ nft, mintbaseNetwork, favo }) => {
        ...
        if (!nftDataLoading) {
            if (data?.nftCollection.edges.length > 0) {
                ...
                if (supaComments.length > 0) {
                    setNftComments(() =>
                        supaComments.map((edge) => {
                            return edge.node
                        })
                    )
                }
                if (supaLikes.length > 0) {
                    setNftLikes(() =>
                        supaLikes.map((edge) => {
                            return edge.node
                        })
    ...
    return (
        <>
            <Box className="p-5 rounded-2xl my-6" bg={postBg}>
                {nftDataLoading ? (
                    <SpinnerContainer />
                ) : (
                    <Box>
                        ...
                        <InteractionBar
                            ...
                            dislikes={
                                nftLikes.filter((like) => like?.value === false).length || '0'
                            }
                            favorite="0"
                            userLike={userLike}
    ...
    return (
        <Modal isOpen={isOpen} size={'5xl'} onClose={onClose}>
            ...
            <ModalContent bg={postBg}>
                <ModalHeader>
                    <Box className="flex rounded-full">
                        <Avatar
                            className=" bg-slate-300 hover:cursor-pointer hover:border-gray-400 hover:border-2"
                            bg={postBg}
                            name={nft.minter}
                    ...
                    <Box m={3} className="flex">
                        <VStack>
                            <ChakraImage
                                ...
                                objectFit="cover"
                            />
                            <Tag color="gray.400" className="ml-3 mr-1" size={'sm'}>
                            ...
                            <Box width={'100%'}>
                                <InteractionBar
                                    likes={
                                        nftLikes.filter((like) => like?.value === true).length ||
                                        '0'
                                    }
                                    dislikes={
                                        nftLikes.filter((like) => like?.value === false).length ||
                                        '0'
                                    }
                                    favorite="0"
                                    userLike={userLike}
                        ...
                        <VStack className=" m-5">
                            <HStack>
                                <Text>Title:</Text>
                                <Text className="font-extrabold" my={2}>
                            ...
                            <CommentSection comments={nftComments} />
                        </VStack>
                    </Box>
                </ModalBody>
