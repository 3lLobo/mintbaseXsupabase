...
export function supabaseInsertNft() {
    const [addNft, { data }] = useMutation(InsertNft, {
...
export function supabaseInsertLike(setUserLike) {
    const [addLike, { data }] = useMutation(InsertLike, {
        client: supabaseClient,
...
export function supabaseInsertComment(setNftComments) {
    const [addComment, { data }] = useMutation(InsertComment, {
        client: supabaseClient,
...
export function supabaseDeleteComment() {
    const [delComment, { data }] = useMutation(DeleteComment, {
        client: supabaseClient,
...
export function supabaseDeleteLike(setUserLike) {
    const [delLike, { data }] = useMutation(DeleteLike, {
        client: supabaseClient,
...
export function supabaseUpdateLike(setUserLike, userLike, addLike) {
    const [updLike, { data }] = useMutation(DeleteLike, {
        client: supabaseClient,
...
export function supabaseFavos(userId, mainnet) {
    ...
    if (error) {
        console.error('supabase QueryFavos error: ', error)
    }
    return { data, loading }
}
...
export function supabaseInsertFavo(setFavo) {
    const [addFavo, { data }] = useMutation(InsertFavo, {
        client: supabaseClient,
...
function deleteFavoHelper(data, setFavo) {
    ...
    setFavo((prev) => {
        return prev.filter((element) => {
            return element !== data.deleteFromFavoriteCollection.records[0]?.id
        })
    })
...
export function supabaseDeleteFavo(setFavo) {
    const [delFavo, { data }] = useMutation(DeleteFavo, {
        client: supabaseClient,
